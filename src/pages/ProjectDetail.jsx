...
const EMPTY_SCREENSHOTS = Object.freeze({});
// ─────────────────────────────────────────────────────────────────────────────
...
const projectMeta = {
  ...
  mindCaps: {
    ...
    en: {
      overview:
        "MindCaps is an AI-powered emotional reflection mobile application. Users describe how they feel in free text, and the app returns structured emotional insights and supportive, non-clinical reflection prompts. The project focuses on AI integration, mobile UX, structured JSON responses, and safe handling of user input rather than medical diagnosis or therapy.",
      architecture:
        "Two-tier architecture. The React Native (Expo) frontend handles user input, local state, and result presentation. AI-related processing is delegated to a Flask REST API backend. The mobile app sends user text to a POST endpoint; the Flask server prepares a structured prompt, processes the AI response, validates the JSON structure, and returns formatted emotional insight to the client. This separation keeps API credentials outside the mobile app and allows the backend logic to evolve without requiring client updates.",
      api:
        "The Flask API exposes a primary endpoint such as `POST /analyze`, accepting user text and language preference. The backend returns a structured JSON response with emotion categories such as joy, sadness, fear, anger, disgust, and surprise, along with a supportive reflection message and a future-self style motivational prompt.",
      security:
        "API credentials are handled server-side and are not exposed in the mobile client. The project is designed to process user input for reflection purposes only. It does not provide diagnosis, treatment, or clinical advice.",
      challenges:
        "The main challenge was producing consistent structured JSON output from free-text emotional input. The solution was to define a strict expected response format and handle parsing carefully so the mobile interface could display emotional insights reliably.",
      role:
        "Full-stack developer — React Native UI, Flask API design, AI response structuring, frontend-backend integration, and bilingual user flow."
    },
    tr: {
      overview:
        "MindCaps, yapay zeka destekli bir duygusal farkındalık mobil uygulamasıdır. Kullanıcılar nasıl hissettiklerini serbest metin olarak yazar; uygulama ise yapılandırılmış duygu içgörüleri ve klinik olmayan destekleyici düşünme mesajları üretir. Proje; tıbbi tanı veya terapi amacı taşımadan yapay zeka entegrasyonu, mobil kullanıcı deneyimi, JSON tabanlı çıktı yapısı ve kullanıcı girdilerinin güvenli işlenmesine odaklanır.",
      architecture:
        "İki katmanlı mimari kullanılmıştır. React Native (Expo) frontend kullanıcı girdisini, yerel durumu ve sonuçların gösterimini yönetir. Yapay zeka ile ilgili işlem akışı Flask REST API backend tarafına taşınmıştır. Mobil uygulama kullanıcı metnini bir POST endpoint’ine gönderir; Flask backend yapılandırılmış bir istem hazırlar, AI çıktısını işler, JSON yapısını doğrular ve formatlanmış duygu içgörüsünü istemciye döndürür.",
      api:
        "Flask API, kullanıcı metni ve dil tercihini kabul eden `POST /analyze` benzeri bir endpoint sunar. Backend; neşe, üzüntü, korku, öfke, tiksinme ve şaşkınlık gibi duygu kategorileriyle birlikte destekleyici yansıtma mesajı ve gelecekteki benlik tarzında motivasyon çıktısı döndürür.",
      security:
        "API bilgileri server-side tutulur ve mobil istemci içinde açık edilmez. Proje yalnızca farkındalık ve refleksiyon amacıyla tasarlanmıştır; tanı, tedavi veya klinik tavsiye sunmaz.",
      challenges:
        "Temel zorluk, serbest metin duygu girişlerinden tutarlı ve yapılandırılmış JSON çıktısı üretebilmekti. Çözüm olarak beklenen çıktı formatı net tanımlandı ve mobil arayüzün duygu içgörülerini güvenilir biçimde gösterebilmesi için parsing süreci dikkatli şekilde ele alındı.",
      role:
        "Full-stack geliştirici — React Native arayüzü, Flask API tasarımı, AI yanıt yapısının kurgulanması, frontend-backend entegrasyonu ve iki dilli kullanıcı akışı."
    },
  },
...
const ProjectDetail = () => {
  ...
  const {
    projectId,
    screenshots: stateScreenshots = EMPTY_SCREENSHOTS,
  ...
  const allProjects = useMemo(() => getProjectsData(t), [t]);
  const fullProjectData = allProjects.find((project) => project.id === id);

  const projectData = fullProjectData || t.projects?.[id];
